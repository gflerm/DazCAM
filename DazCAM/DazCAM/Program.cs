...
namespace DazCAM
{
    public class Program
    {
        ...
        private static bool _errorFlashActive = false;
        ...
        public static void Main()
        {
            ...
            X.OnAxisLimitReached += new Axis.AxisLimitReachedEventHandler(OnAxisLimitReached);
            Y.OnAxisLimitReached += new Axis.AxisLimitReachedEventHandler(OnAxisLimitReached);
            Z.OnAxisLimitReached += new Axis.AxisLimitReachedEventHandler(OnAxisLimitReached);

            EStopButton.OnInterrupt += new NativeEventHandler(EStopButton_OnPress);
            ...
            socketServer.OnRequest += new SocketServer.RequestHandler(SocketServer_OnRequest);
        ...
        private static string RunCommandFile(out bool error)
        {
            using (var reader = new StreamReader(@"\SD\Commands.txt"))
            {
                while (true)
                {
                    string commandBlock = reader.ReadLine();
        ...
        /// path will not be a straight line, all axes are driven until they reach their destination independantly at the specified speed in
        /// pulses per second. This would generally be used as a rapid traverse as it is faster for all steppers to run at full speed than
        ...
        /// format is x,y,z,d where x,y,z are relative steps and s is the delay in milliseconds between steps
        /// </summary>
